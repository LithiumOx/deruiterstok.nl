<script lang="ts">
	...
	let URL_JSQRSCANNER;
	const unsubscribe = LIBRARIES.subscribe((value) => (URL_JSQRSCANNER = value.LIB_JSQRSCANNER));
	...
	const protocol_error_message_short = 'QRCode component requires a secure connection (https).';
		...
		return navigator.mediaDevices
			...
			.then(function (devices) {
				...
				devices.forEach(function (device) {
					if (device.kind === 'videoinput') exCameras.push(device.deviceId);
				});
				return Promise.resolve(exCameras);
			})
			.then(function (ids) {
				if (ids.length === 0) return Promise.reject('Could not find a webcam');
				return navigator.mediaDevices.getUserMedia({
					//this way QQ browser opens the rear camera
					video: {
						// @ts-ignore
						optional: [{ sourceId: ids.length === 1 ? ids[0] : ids[1] }]
					}
				});
			});
	};
	const onQRCodeScanned = (scannedText) => {
		...
		if (scannedTextMemo) {
			...
			scannedTextMemoHist = document.getElementById('scannedTextMemoHist');
	...
	const toggleQRCodeScanning = () => {
		jbScanner.isActive() ? jbScanner.stopScanning() : jbScanner.resumeScanning();
		jbScanner.isActive()
...
<div
	...
	hidden
>
	You may need to serve this page over a secure connection (https) to run JavascriptQRScanner
	correctly.
</div>
		...
		<div class="row-element">
			<div class="FlexPanel detailsPanel QRScannerShort">
				<div class="FlexPanel shortInfoPanel">
					<p>
						To access the QR-Code generator for Token Addresses, click on the profile icon likely a
						random cat picture.
					</p>
					<div class="gwt-HTML">Point the webcam at a QR code.</div>
					<i>QRCode scanner does not record videos, only looks for QRcodes.</i>
					<p />
				</div>
			</div>
			...
			<div class="row-element">
				<div class="form-field form-field-memo">
					<div class="form-field-caption-panel">
						<div class="gwt-Label form-field-caption">Scanned text</div>
					</div>
				...
				<div class="form-field form-field-memo">
					<div class="form-field-caption-panel">
						<div class="gwt-Label form-field-caption">Scanned text history</div>
					</div>
					<div class="FlexPanel form-field-input-panel">
						<textarea
